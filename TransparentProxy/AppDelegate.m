...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    [NETransparentProxyManager loadAllFromPreferencesWithCompletionHandler:
      ^(NSArray<NETransparentProxyManager *> * _Nullable managers, NSError * _Nullable error)
      {
            if (error)
            {
                NSLog(@"loadAllFromPreferencesWithCompletionHandler: %@",
            ...
            for (NETransparentProxyManager *manager in managers)
            {
                [manager removeFromPreferencesWithCompletionHandler:
                    ^(NSError * _Nullable error)
                    {
                        if (error)
                        {
                            NSLog(@"removeFromPreferencesWithCompletionHandler: %@",
                                error.localizedDescription);
                        }
                    }];
            }
            ...
            [manager saveToPreferencesWithCompletionHandler:
                ^(NSError * _Nullable error)
                {
                    if (error)
                    {
                        NSLog(@"saveToPreferencesWithCompletionHandler: %@",
                            error.localizedDescription);
                        return;
                    }
